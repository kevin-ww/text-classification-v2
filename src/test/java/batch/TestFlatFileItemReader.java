...
public class TestFlatFileItemReader<T> extends
   ...
   *          will be called for each one of the initial skipped lines before
   *          any items are read.
   */
   ...
   * 
   * @param encoding
   *          a properties object which possibly contains the encoding for this
   *          input file;
   */
   ...
   */
  public void setBufferedReaderFactory(
      BufferedReaderFactory bufferedReaderFactory) {
    this.bufferedReaderFactory = bufferedReaderFactory;
  }

  /**
   * Setter for comment prefixes. Can be used to ignore header lines as well by
   * using e.g. the first couple of column names as a prefix.
   * 
   * @param comments
   ...
   * line endings are and do things like continue over a line ending if inside a
   * quoted string.
   * 
  ...
  protected T doRead() throws Exception {
    ...
    System.out.println(Thread.currentThread().getName() + " now reading "
        + this.resource);
    ...
    if (line == null) {
      ...
      try {
        ...
        throw new FlatFileParseException("Parsing error at line: " + lineCount
            + " in resource=[" + resource.getDescription() + "], input=["
            + line + "]", ex, line, lineCount);
      }
    }
  }
  ...
  private String readLine() {
    ...
    if (reader == null) {
      throw new ReaderNotOpenException(
  ...
  protected void doOpen() throws Exception {
    ...
    if (!resource.isReadable()) {
      ...
      logger
          .warn("Input resource is not readable " + resource.getDescription());
      return;
    }
      ...
      if (line == null) {
        if (StringUtils.hasText(record)) {
          ...
          throw new FlatFileParseException(
              "Unexpected end of file before record complete", record,
              lineCount);
        } else {
